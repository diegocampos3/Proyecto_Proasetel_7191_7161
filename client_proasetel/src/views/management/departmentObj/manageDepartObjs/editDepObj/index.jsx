...
import { updateObjDep } from 'store/slices/departmentobj'; 
...
const EditDepObj = () => {
    ...
    const objEmpDep = location.state?.objEmpDep
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { setSubmitting }) => {
            try {
                ...
                const result = await dispatch(updateObjDep(initialData.idObjDep, data));

                if (result.success) {
                    setSnackbarMessage('El objetivo fue editado con éxito.');
                    setSnackbarSeverity('success');
                    setTimeout(() => navigate('/management/departmentobj/manageDepartObj',{
                        state :{objEmpDep}
                    }), 1000);
                } else {
                    ...
                    <Grid item xs={12} container justifyContent="flex-end" spacing={2}>
                        <Grid item>
                            <Button
                                variant="outlined"
                                color="secondary"
                                onClick={() => navigate('/management/departmentobj/manageDepartObj',{
                                state :{objEmpDep}
                                })}
                            >
                                Cancelar
